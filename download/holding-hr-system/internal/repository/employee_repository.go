...
func (r *EmployeeRepository) GetStats(companyID *int) (*models.DashboardStats, error) {
        ...
        // Şirkət sayı
        r.db.QueryRow("SELECT COUNT(*) FROM companies").Scan(&stats.TotalCompanies)

        if companyID != nil {
                // Müəyyən şirkət üçün statistika
                // Ümumi say
                r.db.QueryRow("SELECT COUNT(*) FROM employees WHERE company_id = ?", *companyID).Scan(&stats.TotalEmployees)

                // Statuslara görə
                r.db.QueryRow("SELECT COUNT(*) FROM employees WHERE status = 'ACTIVE' AND company_id = ?", *companyID).Scan(&stats.ActiveEmployees)
                r.db.QueryRow("SELECT COUNT(*) FROM employees WHERE status = 'CANDIDATE' AND company_id = ?", *companyID).Scan(&stats.Candidates)
                r.db.QueryRow("SELECT COUNT(*) FROM employees WHERE status = 'TERMINATED' AND company_id = ?", *companyID).Scan(&stats.Terminated)

                // Bu ay işə qəbul
                r.db.QueryRow(`SELECT COUNT(*) FROM employees WHERE status = 'ACTIVE' AND company_id = ? AND MONTH(hire_date) = MONTH(CURRENT_DATE()) AND YEAR(hire_date) = YEAR(CURRENT_DATE())`, *companyID).Scan(&stats.ThisMonthHired)

                // Bu ay işdən çıxan
                r.db.QueryRow(`SELECT COUNT(*) FROM employees WHERE status = 'TERMINATED' AND company_id = ? AND MONTH(termination_date) = MONTH(CURRENT_DATE()) AND YEAR(termination_date) = YEAR(CURRENT_DATE())`, *companyID).Scan(&stats.ThisMonthTerminated)
        } else {
                // Bütün şirkətlər üçün statistika
                // Ümumi say
                r.db.QueryRow("SELECT COUNT(*) FROM employees").Scan(&stats.TotalEmployees)

                // Statuslara görə
                r.db.QueryRow("SELECT COUNT(*) FROM employees WHERE status = 'ACTIVE'").Scan(&stats.ActiveEmployees)
                r.db.QueryRow("SELECT COUNT(*) FROM employees WHERE status = 'CANDIDATE'").Scan(&stats.Candidates)
                r.db.QueryRow("SELECT COUNT(*) FROM employees WHERE status = 'TERMINATED'").Scan(&stats.Terminated)

                // Bu ay işə qəbul
                r.db.QueryRow(`SELECT COUNT(*) FROM employees WHERE status = 'ACTIVE' AND MONTH(hire_date) = MONTH(CURRENT_DATE()) AND YEAR(hire_date) = YEAR(CURRENT_DATE())`).Scan(&stats.ThisMonthHired)

                // Bu ay işdən çıxan
                r.db.QueryRow(`SELECT COUNT(*) FROM employees WHERE status = 'TERMINATED' AND MONTH(termination_date) = MONTH(CURRENT_DATE()) AND YEAR(termination_date) = YEAR(CURRENT_DATE())`).Scan(&stats.ThisMonthTerminated)
        }
